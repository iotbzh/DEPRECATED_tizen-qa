/*
 * TKIT Database class
 * Authors: Stéphane Desneux <stephane.desneux@open.eurogiciel.org>
...
var logger=log4js.getLogger("testdb");
...
var Database=module.exports=function(basedir) {
	...
	try {
		...
		this.root.refresh();
		DEBUG && logger.debug("Creating tests db from basedir "+basedir);
	}
	catch(error){
		logger.error("testdb: invalid database dir '"+basedir+"'"+"\n"+error.message);
		process.exit(1)
	}
}
...
/*
Return an Array of objects of type TKIT.Testset. 
The TKIT.Testset returned are those that are in the
subdirectory 'subdir' of the root directory of the database.
The function 'filterTestcaseFunc' is passed to the 'applyFilter'
method of the selected TKIT.Testset to remove unwanted test.
The several ways of calling the function:
...
Database.prototype.getTestsets=function(subdir,filterTestcaseFunc) {
	...
	this.root.refresh().get(subdir).walkDown(function(tdir){
		...
		var tset = new TKIT.Testset(tdir);
		tdir.forEachQaMeta(function(filename,name){
			...
			tset.addCases(TKIT.utils.splat(obj),name);
		});
...
var TestDir = Database.TestDir = function(dirpath,name,upper) {
	...
	if (!name && !upper) {
		...
		if (!TKIT.utils.isTestDir(name))
			throw new Error("The name '"+name+"' isn't a valid test directory name.");

		this.name = name;
		this.shortname = TKIT.utils.stripName(name);
		this.prefix = TKIT.utils.getPrefix(name);
		this.upperDir = upper;
		if (upper.isRoot()) {
			this.path = name;
...
TestDir.prototype.__check_and_add_if_needed = function(subname) {
	if (TKIT.utils.isTestDir(subname)) {
