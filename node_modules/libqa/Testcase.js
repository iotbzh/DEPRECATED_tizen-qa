...
var logger=log4js.getLogger("qatcase");
...
var deftcconfig = {
    ...
    bugs: "",
	 pre_condition: "",
	 steps: [], // an array of objects containing { description: "xxx", expected: "yyy" }
	 post_condition: "",
...
Parameters:
 - testset (Object: QA.Testset): the testset that records this test case.
...
The built object contains all the fields listed above plus:
 - testset (Object: QA.Testset): the given testset
...
var Testcase=module.exports=function(testset,tcasenum,definition,filename) {
	if (!(testset instanceof QA.Testset)) {
	...
	this.name=QA.utils.encodeName(tcasenum,this.name);
...
Testcase.prototype.toExportable=function() {
	...
	var obj={
		...
		shortname:					QA.utils.stripName(this.name),
		fullname:					this.getComponentName() + "/" + QA.utils.stripName(this.name),
		name:							this.name,
		...
		pre_condition:				t(this.pre_condition),
		post_condition:			t(this.post_condition),
		steps: 						this.steps.map(function(s) { 
											var res={};
											_.keys(s).forEach(function(k) {
												res[k]=t(s[k]);
												});
											return res;
										}),
